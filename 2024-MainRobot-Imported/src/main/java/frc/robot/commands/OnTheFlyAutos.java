  ...
  public Command getOnTheFlyAuto(Pose2d initPose, String navgrid, int... places){
    ...
    return Commands.defer(()->Commands.runOnce(()->{
        ...
        if (!DriverStation.getAlliance().isEmpty() && DriverStation.getAlliance().get() == Alliance.Red){
            drivetrain.resetPose(new Pose2d(16.54-initPose.getX(),initPose.getY(), Rotation2d.fromDegrees(180-initPose.getRotation().getDegrees())));
            return;
        }
        drivetrain.resetPose(initPose);
