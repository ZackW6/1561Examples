...
//TODO there may be some mathematical problems, but as of now works
public class ObjectDetectionSim implements ObjectDetectionIO{
    ...
    public Optional<Pose3d> getClosestVisiblePiece(){
        ...
        for (Pose3d pose : algaeSubscriber.get()){
            ...
            if (pitchFromPiece.getRadians()<Units.degreesToRadians(fieldOfView.get(1,0)/2)
                    && pitchFromPiece.getRadians()>-Units.degreesToRadians(fieldOfView.get(1,0)/2)
                    && yawFromPiece.getRadians()<Units.degreesToRadians(fieldOfView.get(0,0)/2)
                    && yawFromPiece.getRadians()>-Units.degreesToRadians(fieldOfView.get(0,0)/2)
                    && dist<6){
                objects.add(pose);
            }
