...
public class WaitAutos {
    ...
    private HashMap<String, Pose2d> createdPaths = new HashMap<>();

    private final CommandMechanism mainMechanism;

    public WaitAutos(CommandMechanism mainMechanism){
     ...
     */
    public Command createBranchCommand(String nameOfAuto, Pose2d beginPose, String dynamicObstacles, boolean doAlgae, BranchInstruction... branchInstructions){
    ...
    private Command tryPath(String from, String to, int place, int layer, boolean avoidObstacles){

        if (avoidObstacles){
            if (to.startsWith("P")){
                return mainMechanism.autoScoreCoral(place, layer);
            }
            try {
                return mainMechanism.autoIntakeCoral(Integer.parseInt(to.substring(1,2),10),0);
            } catch (Exception e2) {
        ...
        if (to.startsWith("P")){
            ...
            return Commands.defer(()->{
                try {
                    return mainMechanism.autoScoreCoral(AutoBuilder.followPath(PathPlannerPath.fromChoreoTrajectory(from+to)), place, layer, .2);
                } catch (Exception e) {
                    return mainMechanism.autoScoreCoral(place, layer);
                }
            },mainMechanism.mainSubsystems);
        }
        return Commands.defer(()->{
            try {
                return mainMechanism.autoIntakeCoral(AutoBuilder.followPath(PathPlannerPath.fromChoreoTrajectory(from+to)), Integer.parseInt(to.substring(1,2),10),0,2);
            } catch (NumberFormatException | FileVersionException | IOException | ParseException e) {
                return mainMechanism.autoIntakeCoral(Integer.parseInt(to.substring(1,2),10),0);
            }
        }, mainMechanism.mainSubsystems);
    }

    // private Command autoScoreCoral(Command path, int place, int level, double straightDist){
    ...
    //         , FactoryCommands.maxSpeedAutoCoral, 3*Math.PI, 5), Set.of()))
    ...
    //         return comparingTransform.getTranslation().getNorm() < FactoryCommands.raiseElevatorDist ? 1/(Math.min(comparingTransform.getTranslation().getNorm(),5)/5): 0;
    //     })).andThen(Commands.waitSeconds(.1)).andThen(factoryCommands.scoringMechanism.scoreCoral(level)));
    // }

    // private Command autoIntakeCoral(Command path, int place, double straightDist){
    //     return Commands.race((path.alongWith(Commands.waitSeconds(.4).andThen(factoryCommands.scoringMechanism.resetElevator())))
    //         .until(()->factoryCommands.drivetrain.getPose()
    ...
    //         , FactoryCommands.maxSpeedAutoIntake, 3*Math.PI, 5),Set.of()),factoryCommands.scoringMechanism.intake())));
    // }
