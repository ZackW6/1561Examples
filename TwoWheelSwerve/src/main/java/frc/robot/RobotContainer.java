...
public class RobotContainer {
  ...
  private final SwerveDrive drive = DriveBaseConstants.talonDrive;

  private final double maxSpeed = DriveBaseConstants.speedAt12Volts;

  private final double maxRotationRad = 3 * Math.PI;

  private void configureBindings() {
    drive.registerTelemetry(logger::telemeterize);
    drive.setDefaultCommand(drive.fieldRelativeDrive(()->-driverController.getLeftY()*maxSpeed
      ,()->-driverController.getLeftX()*maxSpeed,()->-driverController.getRightX()*maxRotationRad));
